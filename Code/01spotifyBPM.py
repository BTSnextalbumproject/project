...
# Spotify API 인증 정보
cid = 'e31852c2b28e45bc8c28d0d8bc78c0fb'
secret = 'b0e456f8cb63441188b3154ce5d63bc3'
    ...
    while True:
        try:
            ...
            if e.http_status == 429:
                ...
                print(f"{song} - {artist}의 BPM을 가져오는 중 오류 발생: {e}")
                break
    return None
...
def process_songs_in_batches(input_file, output_file, batch_size=100, start_idx=0):
    ...
    for i in range(start_idx, len(df_songs), batch_size):
        ...
        for idx, row in batch_songs.iterrows():
            if pd.notna(row['BPM']):
                # BPM 값이 이미 있는 경우 API 요청을 건너뜀
                print(f"{row['Title']} - {row['Artist']}은(는) 이미 처리됨, BPM: {row['BPM']}")
                continue

            song = row['Title'].strip()
            artist = row['Artist'].strip()
            bpm = get_bpm(song, artist)
            
            # BPM을 가져온 후에 None이 아닌 경우에만 저장
            if bpm is not None:
                df_songs.at[idx, 'BPM'] = bpm
                print(f"{song} - {artist} 처리 완료, BPM: {bpm}")
            else:
                print(f"{song} - {artist}의 BPM 정보를 찾을 수 없습니다.")
            
            time.sleep(1.5)  # 요청 제한 방지
...
# 함수 호출
process_songs_in_batches('./billboardTop100Totalduplicate.csv', './billboardTop100Totalduplicate_with_BPM.csv')